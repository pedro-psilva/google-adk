...
def build_expected_signals(bundle: dict[str, Any]) -> list[ExpectedSignal]:
    ...
    if dominant_style:
        # Split multi-style labels (e.g. "Comunicador Planejador") into individual signals
        # so coverage verification checks each style independently.
        ...
        for style_name in individual_styles:
            signals.append(
                ExpectedSignal(
                    ...
                    metadata={"scores": profiler.get("scores", []), "dominant_style": dominant_style},
                )
            )
