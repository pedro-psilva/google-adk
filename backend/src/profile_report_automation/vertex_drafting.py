...
def build_generation_payload(bundle: dict[str, Any], coverage: dict[str, Any]) -> dict[str, Any]:
    ...
    user_prompt = (
        ...
        "- use o campo `dominant_style` da fonte estruturada como o estilo oficial do Profiler; "
        "quando contiver mais de um nome (ex: 'Comunicador Planejador'), mencione todos eles "
        "na secao de Profiler e na conclusao\n\n"
        "Regras para textos existentes (campo `existing_sections`):\n"
        "- quando a secao `profiler` ja tiver texto preenchido, use-o como base e reescreva "
        "apenas para adequar o tom; nao substitua o estilo identificado\n"
        "- quando a secao `conclusion` contiver itens numerados (iniciando com '1-', '2-' etc.), "
        "PRESERVE esses itens exatamente como estao, na mesma ordem e quantidade; "
        "nao gere uma conclusao alternativa nem reduza o numero de itens\n"
        "- quando a secao `career_anchors` ou `cultural_diagnosis` ja tiver texto preenchido, "
        "use-o como referencia principal; nao omita ancoras ou culturas ja citadas\n\n"
