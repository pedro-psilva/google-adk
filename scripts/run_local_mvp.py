...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compatibility wrapper for the production backend pipeline.")
    parser.add_argument("bundle_input", help="Path to a normalized bundle JSON or an intake folder")
    parser.add_argument("--output-dir", default="artifacts/local-mvp", help="Where to write generated artifacts")
    parser.add_argument(
        "--vertex-mode",
        choices=["preview", "live"],
        default="preview",
        help="preview writes the Vertex request and uses the local fallback draft; live calls Vertex AI.",
    )
    args = parser.parse_args()

    result = ReportPipelineService().run(
        PipelineRequest(bundle_input=args.bundle_input, output_dir=args.output_dir, draft_mode=args.vertex_mode)
    )
    print(
        {
